...
def test_model(epi, epi2, data, train, reward, reward2, agent_switch):
    ...
    data = pd.read_csv(r'data\data_2023-2.csv')
            ...
            if action == 0:  # Open a buy option
                ...
                for position in self.positions:
                    if position['type'] == 'buy':
                        ...
                        if buy_profit > 0:
                            buy_profit_amt = buy_profit * .90
                            vault_amt = buy_profit * .10
                            self.vault += vault_amt 
                ...
                for position in self.positions:
                    if position['type'] == 'sell':
                        ...
                        if sell_profit > 0:
                            sell_profit_amt = sell_profit * .90
                            vault_amt = sell_profit * .10
                            self.vault += vault_amt
                        self.portfolio['total_free_margin'] += position['total_margin']
    ...
    while episode_count < total_episodes:
        ...
        failure_done = False
        while not done:
            if env.portfolio['total_equity'] < (10000*0.50) or env.portfolio['total_balance'] < (10000* 0.50):  # Define a custom method in your environment to determine if the episode is done
                done = True
                failure_done = True
                env.reset() 
                episode_reward = 0
