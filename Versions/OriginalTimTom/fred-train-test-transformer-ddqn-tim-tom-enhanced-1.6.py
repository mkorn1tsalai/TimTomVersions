...
def test_model(epi, epi2, data, train, reward, reward2, agent_switch):
    ...
    data = pd.read_csv(r'data\data_2025_1m.csv')
    ...
    moving_averages = [60, 60*2, 60*3, 60*4, 60*5, 60*6]
    ...
    start_index = random.randint(0, total_observations - 60)
    ...
    class TradingEnvironment:
        ...
        def step(self, action, reward, reward2, agent_switch):
            ...
            return next_state, reward, reward2, total_bal, original_action, action_factor, done
    ...
    batch_size = 60 # Adjust the batch size for experience replay
    batch_size2 = 60*24
    episode_rewards = []
    episode_count = 0
    episode_reward = 0
    ...
    while episode_count < total_episodes:
        ...
        while not done:
            if env.portfolio['total_equity'] < (10000*0.90) or env.portfolio['total_balance'] < (10000* 0.90):  # Define a custom method in your environment to determine if the episode is done
                ...
                next_state, reward, reward2, total_bal, original_action, action_factor, done = env.step(action, reward, reward2, agent_switch)  # Implement the step method in your environment
...
dataset = pd.read_csv(r'data\itd_2000_2024.csv')
train = True
...
while train == True:
        ...
        def step(self, action, reward, reward2, agent_switch):
            ...
            if action == 0 or action == 1:
              ...
              if action != 4:
                ...
                rrc = round(rrc,2)
                if rrc > .10:
                  print("Change Action 2")
                  print("the rrc is", rrc)
                  if action == 0:
                    action = 4
                    action_factor = -1
                  if action == 1:
            ...
            return next_state, reward, reward2, total_bal, original_action, action_factor, self.reset1, self.reset2,trade_bias, done
    ...
    class DQNAgent:
        ...
        def replay(self, batch_size, batch_size1, batch_size2, batch_size3, batch_size4, batch_size5, batch_size6, agent_switch):
            ...
            if agent_switch == 1:
              ...
              rewards = np.array(rewards)  # Convert rewards to NumPy array
              dones = np.invert(np.array(dones)).astype(int)
              targets = rewards + self.gamma * np.max(self.model.predict(next_states), axis=1) * np.invert(dones).astype(int)
              target_values = self.model.predict(states)
              target_values[np.arange(len(target_values)), actions] = targets
              ...
              rewards = np.array(rewards)  # Convert rewards to NumPy array
              dones = np.invert(np.array(dones)).astype(int)
              targets2 = rewards + self.gamma * np.max(self.model2.predict(next_states), axis=1) * np.invert(dones).astype(int)
              target_values2 = self.model2.predict(states)
              target_values2[np.arange(len(target_values2)), actions] = targets2
              self.model2.fit(states, target_values2, epochs=1, verbose=0)
              ...
              rewards = np.array(rewards)  # Convert rewards to NumPy array
              dones = np.invert(np.array(dones)).astype(int)
              targets = rewards + self.gamma * np.max(self.model2.predict(next_states), axis=1) * np.invert(dones).astype(int)
              target_values = self.model2.predict(states)
              target_values[np.arange(len(target_values)), actions] = targets
    ...
    total_episodes = len(data)  # Set the total number of episodes based on your dataset
    batch_size = 120  # Adjust the batch size for experience replay
    batch_size1 = 240  # Adjust the batch size for experience replay
    batch_size2 = 480
    batch_size3 = 720
    batch_size4 = 960
    batch_size5 = 1440
    ...
    while episode_count < total_episodes:
        ...
        while not done:
            if env.portfolio['total_equity'] < (10000 * 0.90) or env.portfolio['total_balance'] < (10000 * 0.90):  # Define a custom method in your environment to determine if the episode is done
                ...
                state = np.reshape(state, [1, state_size])
                action = dqn_agent.act(state)
                next_state, reward, reward2, total_bal, original_action, action_factor,reset1, reset2, trade_bias, done = env.step(action, reward, reward2, agent_switch)  # Implement the step method in your environment
                next_state = np.reshape(next_state, [1, state_size])
                episode_balance = total_bal
                episode_reward += reward
                episode_reward2 += reward2
        ...
        if len(env.positions)<=0:
          if action == 2 or action == 3 or action == 5:
              dqn_agent.replay(batch_size, batch_size1, batch_size2, batch_size3, batch_size4, batch_size5, batch_size6, agent_switch)
        dqn_agent.epi_decay()
        if failure_done == True:
            break
